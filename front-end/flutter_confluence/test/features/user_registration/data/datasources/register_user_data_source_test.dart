...
void main() {
  ...
  setUp(() {
    ...
    dataSource =
        RegisterUserDataSourceImpl(auth: mockAuth, client: mockHttpClient);
  });

  User _createFirebaseUser(
  ...
  UserCredential _createUserCredentials(
      String uid, String displayName, String email, String photoUrl) {
  ...
  group('register user in firebase', () {
    ...
    test('should return the user when email and password are valid', () async {
      ...
      final mockCredential =
          _createUserCredentials(uid, displayName, email, photoUrl);
  ...
  group('create user in custom backend', () {
    ...
    final SkillsModel skills =
        SkillsModel(primarySkills: const [], secondarySkills: const []);
